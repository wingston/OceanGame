...
public class RunLengthEncoding {
  ...
  private DList<TypeAndSize> runs;
  private DListNode<TypeAndSize> current;
  ...
  public RunLengthEncoding(int i, int j, int starveTime) {
    ...
    current=new TypeAndSize(Ocean.EMPTY,i*j);
  ...
  public RunLengthEncoding(int i, int j, int starveTime,
	  ...
	  runs = new DList<TypeAndSize>();
	    for (int k = 0; k < runTypes.length; k++) {
	    TypeAndSize run = new TypeAndSize(runTypes[k],runLengths[k]);
	      runs.insertBack(run);
	    }
